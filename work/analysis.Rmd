...
conf_mat5 <- table(predicted5, Instruments$instrument)
conf_mat5
MisclassRate5<-1-sum(diag(conf_mat4))/nrow(Instruments)
MisclassRate5
IndividualMisclassRates5<-rep(NA, 6)
for (i in 1:6){
  Predictions<-conf_mat5[,i]
  NumberPredictions<-sum(Predictions)
  NumberIncorrect<-sum(Predictions[-i])
  IndividualMisclassRates5[i]<-NumberIncorrect/NumberPredictions
}
IndividualMisclassrates5<-data.frame(IndividualMisclassRates5)
`rownames<-`(IndividualMisclassRates5, INSTRUMENTS)
```
